...
G.ClonerScript.prototype.realize = function(){
  ...
  this.params.rotRange =  this.params.rotRange ||  {x: {start: 0, end: 0}, y: {start: 0, end: 0}, z:{start: 0, end: 0 }}
...
G.ClonerScript.prototype.spawnPrimitives = function(){
  for(var i = 0; i < this.params.num; i++){
    ...
    primitive.transform.rotation.set(G.rf(this.params.rotRange.x.start, this.params.rotRange.x.end), G.rf(this.params.rotRange.y.start, this.params.rotRange.y.end), G.rf(this.params.rotRange.x.start, this.params.rotRange.x.end))
    this._object.addChild(primitive);
    _.each(this.params.primitiveEffectors, function(effector){
      if(effector.name === 'visibility'){
